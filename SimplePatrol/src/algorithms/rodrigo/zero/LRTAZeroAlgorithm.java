...
public class LRTAZeroAlgorithm extends SimpleMultiagentAlgorithm {
	...
	public LRTAZeroAlgorithm(String name, int valueUpdateRule, boolean synchronizeAllNodes, boolean updateTargetNodeBeforeLeaving) {
		super((updateTargetNodeBeforeLeaving? "EZr1" : "Zr") + "(" + name + "," +(synchronizeAllNodes? "g" : "n") + ")" );
	...
	public LRTAZeroAlgorithm(String name, boolean synchronizeAllNodes, boolean updateTargetNodeBeforeLeaving) {
		super((updateTargetNodeBeforeLeaving? "EZr1" : "Zr") + "(" + name + "," +(synchronizeAllNodes? "g" : "n") + ")" );
	...
	public SimpleAgent[] createTeam(AgentPosition[] positions, Graph g, int time) {
		...
		for (int i = 0; i < numAgents; i++) {
			agents[i] = new LRTAZeroAgent(g, blackboard, valueUpdateRule, updateAllNodes, updateBeforeLeaving);
		}
		return agents;
	}
