...
public class LRTAZeroAgentV2 extends SimpleAgent {
	...
	public LRTAZeroAgentV2(Graph g, NodesMemories<int[]> blackboard, int valueUpdateRule, boolean updateAllNodes, boolean updateTargetNodeBeforeLeaving) {
		super(System.out);
		this.g = g;
	...
	public int onArrivalInNode(int nextTurn) {
		...
		// Searchs for the mininum U neighbor from targetNode in currentNodeMemory (look ahead)
		int secondTargetNode = findTargetNode(currentNodeMemory, targetNode);
		int secondTargetNodeMinU = currentNodeMemory[secondTargetNode];
		...
		case LRTAZeroAlgorithm.DEFAULT:
			...
			if (updateTargetBeforeLeaving) {
				// Update targetNode in currentNodeMemory before leaving
				currentNodeMemory[targetNode] = secondTargetNodeMinU + 1;
		...
		case LRTAZeroAlgorithm.WAGNER_RULE:
			...
			if (updateTargetBeforeLeaving) {
				if (currentNodeMemory[targetNode] <= secondTargetNodeMinU) {
					// Update targetNode in currentNodeMemory before leaving
		...
		case LRTAZeroAlgorithm.THRUN_RULE:
			// Update current node value in the memory of the node itself
			currentNodeMemory[currentNode] = Math.max(currentNodeMemory[currentNode] + 1, minU + 1);
			agentMemory[currentNode] = currentNodeMemory[currentNode]; // update agent's memory

			if (updateTargetBeforeLeaving) {
				// Update targetNode in currentNodeMemory before leaving
				currentNodeMemory[targetNode] = Math.max(currentNodeMemory[secondTargetNode] + 1, secondTargetNodeMinU + 1);
				agentMemory[targetNode] = currentNodeMemory[targetNode];
			}
	...
	// Finds the neighbor node from sourceNode with minimum U
