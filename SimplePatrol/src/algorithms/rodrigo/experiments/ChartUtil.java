...
public class ChartUtil {
	...
	public void generateCharts(Map<String,String> chartTitles, ArrayList<ExperimentReport> reports, int width, int height, Map<String,String> algorithms, int[] agentNumbers, 
		...
		for(ExperimentReport report : reports){
			charts.add(generateChartsReworked(chartTitles.get(report.getMapName()), report, algorithms, agentNumbers, relativeValues, excludeFromMin, grayScale, rangeAxisUpperBound, metricName));
		}
	...
	public JFreeChart generateChartsReworked(String chartTitle, ExperimentReport experimentReport, Map<String,String> algorithms, int[] agentNumbers, 
		...
		ArrayList<AlgorithmReport> algorithmReports = experimentReport.getAlgorithmReports();
						...
						for (int j = 0; j < agentNumbers.length; j++) {
							if (agentSets[i] == agentNumbers[j]) {
								if (relativeValues) {
									...
									serie.add(agentSets[j], algReport.getMetrics().get(metricIndex).getMean(j));
								}
		...
		for (JFreeChart chart : charts) {
			...
			if(true){
				File file = new File(experimentReport.getMapName() + " " + chart.getTitle().getText() + ".png");
				...
				Writer out;
				try {
					out = new FileWriter(experimentReport.getMapName() + " " + chart.getTitle().getText() + ".eps");
					out.write(g.toString());
					out.close();
				} catch (IOException e) {
					// TODO Auto-generated catch block
