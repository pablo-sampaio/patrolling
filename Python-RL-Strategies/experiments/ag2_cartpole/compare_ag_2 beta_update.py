...
base_filename = curr_directory + "/results/ag2_beta_update.py-SET30-"
nstep_list = [1, 8, 21, 32] 
#nstep_list = [1, 4, 8, 16, 24, 32]

#"""
# the performance of each beta_update (for varied nsteps)
# for each beta_update, shows one graph, comparing its performances for all n_steps
for beta_update in [21, 41, 22, 42, 23, 43, 44, 37, 38]:
#for beta_update in [21, 41]:
    list_results = []
    ...
    ex.plot_results_comparison(list_results, 
                                title=f"Comparison for update{beta_update}",
                                ylim=[-50000, 10])
#"""
