...
from patrolling_agents.rl_agent_2_gym import ContinuingPGradientAgent

ex.set_results_dir(curr_directory + "/results")


class AcrobotWithFinalReward(Wrapper):
    def __init__(self):
        super().__init__(ContinuingTaskVersion(AcrobotEnv(), +500))
...
for nstep in nstep_list:
    ...
    agent_parameters = {
        ...
        "critic_lr"     : 0.16914,
        "beta"          : 0.01349,
        "beta_update"   : [21, 22, 37, 38, 41, 42, 43, 44, 45, 46], #[21, 22, 23, 37, 38, 41, 42, 43, 44],  # best: 21 / here: 41/21 (and 37 and 38)
        "n_step"        : nstep  # best: 9 / here: 3/4
    }

    results_filename = basefilename_format.format(nstep)

    ex.run_batch(AcrobotWithFinalReward, ContinuingPGradientAgent, 
                env_parameters, 
                agent_parameters, 
                experiment_parameters,
                'beta_update',
