...
import exploration.relative_order.experimental.DfsExplorer5Agent.Move;
 ...
 */
public class DfsExplorer6Multi extends MultiagentExplorationWithRelativeOrder {
	
	public DfsExplorer6Multi() {
		super("DfsExplorer-6-multi-new");
	}
	...
	public ExplorerAgentRelativeOrder[] createTeam(int agents, int totalTime) {
		...
		for (int i = 0; i < agents; i++) {
			team[i] = new DfsExplorer6Agent(System.out, sharedNavigationData);
		}
		return team;
	}
...
class DfsExplorer6Agent extends ExplorerAgentRelativeOrder {
	...
	enum Move { START, FORWARD, BACKTRACK, FINISH, EXPAND_FWD, EXPAND_BACK };
	...
	DfsExplorer6Agent(PrintStream teamLog, Map<Integer,NodeMemory2> sharedNodeData) {
		super(teamLog);
	...
	public int endForwardMove(int nodeId, NodeMemory2 nodeData, boolean firstTime, int nextTurn) {
		...
		if (nodeData.isComplete() || visitedByMe) {
			...
			if (nodeData.getExitTo(lastNode) >= 0) {
				...
				this.explAncestorData.put(nodeId, this.explNextElement_fwd);     //store backtrack data (only in agent memory)
				return beginExpandForward(nodeId, tmpNodeData, referenceExit);   //faz um forward (pode ser com o tmpNodeData)
			}
	...
	private int beginExpandForward(int nodeId, NodeMemory2 nodeData, int referenceExit) {
		...
		print("- expand (can-exit #" + nextCanonExit + ", rel-exit #" + nextRelatExit + ")");
		
		this.explNextElement_fwd = new Pair<>(nodeId, nextCanonExit);
		this.currentMove = Move.EXPAND_FWD;
	...
	public int endExpandForward(int nodeId, NodeMemory2 nodeData, int nextTurn) {
		int lastNode = this.explNextElement_fwd.first;
		...
		//      encerrar um no completado (testar se pode fazer merge, ou se tem vizinho 1), etc

		return beginExpandBack(nodeId, this.explNextElement_fwd);
	}
	
	private int beginExpandBack(int nodeId, Pair<Integer,Integer> ancestorInfo) {
		int ancestorNode = ancestorInfo.first;
		int nextRelatExit = 0;  //return through the 'entrance' edge (for already-visited nodes reached in a forward move, because the reference is not properly set in this case)
		print("- expand back to " + ancestorNode +" (rel-exit #0)");
		...
		this.currentMove = Move.EXPAND_BACK;
		this.explNextElement_fwd = null;
		return nextRelatExit; 
	}

	public int endExpandBack(int nodeId, NodeMemory2 nodeData, int nextTurn) {
		int referenceExit = this.backtrackedExit_bkt;

		assert (this.tempNavData.containsKey(nodeId));
		...
		//TODO: tenho que mudar a estrutura, para ser capaz de diferenciar entre saidas expandidas x exploradas (usar a "List2?")
		//IDEIA: usar o status do no tambem (como a List2)
