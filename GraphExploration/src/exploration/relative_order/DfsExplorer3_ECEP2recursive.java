 ...
 */
public class DfsExplorer3_ECEP2recursive extends MultiagentExplorationWithRelativeOrder {
	private PrintStream output;
	
	public DfsExplorer3_ECEP2recursive() {
		this(false);
	}
	
	public DfsExplorer3_ECEP2recursive(boolean verboseMode) {
		super("DfsExplorer-3-ECEP-v2-recursive");
		if (verboseMode) {
	...
	public ExplorerAgentRelativeOrder[] createTeam(int agents, int totalTime) {
		...
		for (int i = 0; i < agents; i++) {
			team[i] = new DfsExplorer3Agent_ECEP2_Rec(this.output, sharedNavData);
		}
		return team;
	}
...
class ECEPThread extends Thread {
	private int effectivePort;
	private NodeMemory0 nodeMemory;
	private int turn;
	private boolean exploring;
	private DfsExplorer3Agent_ECEP2_Rec agent;
	
	ECEPThread(DfsExplorer3Agent_ECEP2_Rec ag) {
	...
	public void run() {
		try {
			...
			V.lastPort = 0;
			V.nav[0] = NodeMemory0.IN_EXPLORATION;
			...
			while ((V.lastPort + 1) < V.delta) {
				...
				V.nav[V.lastPort] = NodeMemory0.IN_EXPLORATION;
	...
	private void DFS_visit(NodeMemory0 V, int prevNode) throws InterruptedException {
		boolean firstVisit = (V.nav[0] == NodeMemory0.UNEXPLORED);
...
class DfsExplorer3Agent_ECEP2_Rec extends ExplorerAgentRelativeOrder {
	...
	private ECEPThread ecep;
	
	DfsExplorer3Agent_ECEP2_Rec(PrintStream teamLog, Map<Integer,NodeMemory0> sharedNodeData) {
		super(teamLog);
